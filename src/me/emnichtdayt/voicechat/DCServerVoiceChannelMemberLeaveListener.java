	...
	public void onServerVoiceChannelMemberLeave(ServerVoiceChannelMemberLeaveEvent event) {
		...
		if(targetVoice!=null) {
			if(targetVoice.getState()==VoiceState.CONNECTED) {
				if(!(optionalNChannel.isPresent()&&optionalNChannel.get().getName().length()>8&&optionalNChannel.get().getName().substring(0, 9).equals("VoiceChat"))&&!(optionalNChannel.isPresent()&&optionalNChannel.get().getId()==Long.parseLong(VoiceChatMain.getDcbot().getWaitingChannelID()))) {
					if(VoiceChatMain.getVoiceChatRequired()) {
						Player target = targetVoice.getPlayer();
						targetVoice.setState(VoiceState.DISCONNECTED);
						VoiceChatMain.fireVoiceStateChange(targetVoice, VoiceState.CONNECTED, VoiceState.DISCONNECTED, true);
						...
						if(targetVoice.getCurrentChannel()!=null) {
							...
							VoiceChatMain.fireVoiceStateChange(targetVoice, VoiceState.CONNECTED, VoiceState.DISCONNECTED, false);
